...
export default class CustomDrawer extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (!equals(nextProps.route, this.props.route)) {
      ...
      this.state.open &&
      !contains(nextProps.side, nextProps.openDrawers)
    ) {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      !equals(this.state.open, nextState.open) ||
  ...
  _toggleDrawer(dynamicName = null) {
    const status =
      !dynamicName || !this.contentName || equals(dynamicName, this.contentName)
        ? !this.state.open
        : !equals(dynamicName, this.contentName) && !this.state.open
        ? true
        : this.state.open;
  ...
  render() {
    ...
    const classNames = [...this.classNames, this.state.open ? '-open' : ''];
      ...
      <Flexbox className={concatClasses(classNames)}>
        {side === RIGHT && (
          <Flexbox
            ...
            <Icon
              icon="arrow_left"
              className={this.state.open ? 'rotate_180' : ''}
              size="sm"
            />
          </Flexbox>
        )}
